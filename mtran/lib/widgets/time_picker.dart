...
class _AppTimePickerState extends State<AppTimePicker> {
  ...
  void initState() {
    ...
    if (widget.value != null) {
      ...
      widget.controller?.text =
          DateFormat(widget.formatSpecifier ?? 'jm').format(widget.value!);
      setState(() {
        changedTimeIOS = widget.value!;
      });
      ...
      if (parsed != null) {
        selectedTime = parsed;
        changedTimeIOS = parsed;
      }
    }

    // Always show placeholder if no time selected
  ...
  Future<void> pickTime() async {
    ...
    if (!kIsWeb) {
          ...
          Widget content = Container(
            ...
            child: Column(
              ...
              children: [
                SizedBox(
                  height:
                      AppSpacing.kDefaultSpacing(context, useWatch: false) * 20,
                  child: CupertinoDatePicker(
                    ...
                    onDateTimeChanged: (DateTime newDateTime) {
                      widget.controller?.text =
                          DateFormat(widget.formatSpecifier ?? 'jm')
                              .format(newDateTime);
                      setState(() {
                ...
                Container(
                  ...
                  child: AppButton(
                    ...
                    onPressed: () {
                      widget.controller?.text =
                          DateFormat(widget.formatSpecifier ?? 'jm')
                              .format(changedTimeIOS);
                      setState(() {
                        selectedTime = changedTimeIOS;
                      });
      ...
      widget.controller?.text =
          DateFormat(widget.formatSpecifier ?? 'jm').format(changedTimeIOS);
      setState(() {
        isLoading = false;
        selectedTime = changedTimeIOS;
      });
            ...
            : (widget.glass
                ...
                : useThemeMode(
                    ...
                    light: widget.gradient
                        ...
                        : getThemeColor(context)?.shade200.withValues(
                              alpha: 0.5,
                            ),
